...
const schema = z.object({
  ...
  sujet: z.enum(TYPES, { errorMap: () => ({ message: "Sélectionnez un type de demande" }) }),
...
const ContactForm = ({ defaultSujet }: ContactFormProps) => {
  ...
  useEffect(() => {
    if (defaultSujet) setValue("sujet", defaultSujet, { shouldValidate: false });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="card-premium p-8 md:p-10 space-y-5">
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        ...
        <Field label="Téléphone *" error={errors.telephone?.message}><Input type="tel" {...register("telephone")} /></Field>
      </div>
      <Field label="Société *" error={errors.societe?.message}><Input {...register("societe")} /></Field>
      <Field label="Type de demande *" error={errors.sujet?.message}>
        <Select
          value={sujetValue ?? ""}
          onValueChange={(v) => setValue("sujet", v as (typeof TYPES)[number], { shouldValidate: true })}
        >
      ...
      <Field label="Message *" error={errors.message?.message}>
        <Textarea rows={6} placeholder="Décrivez brièvement votre contexte, vos enjeux et l'horizon de votre projet." {...register("message")} />
      </Field>
      ...
      <div className="pt-4 border-t border-border grid sm:grid-cols-3 gap-4 text-xs text-muted-foreground">
        <div className="flex items-center gap-2"><Clock className="h-4 w-4 text-champagne" /> Réponse sous 24 à 48h</div>
        <div className="flex items-center gap-2"><Lock className="h-4 w-4 text-champagne" /> Données confidentielles</div>
